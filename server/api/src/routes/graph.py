...
# Note: "subcats" here mean the "Subcategories of Computer Science" (eg, those visible to the UI)
# not the subcategories of those
    ...
    for name, _, cats in all_courses:
        for cat in cats:
            if cat in cats_to_subcats:
                ...
                for subcat in cats_to_subcats[cat]:
...
def build_graph(
    all_courses, all_courses_names, cats_to_subcats, subcats_to_courses, cats_to_courses
):
    adj_matrix = {
...
def generate_graph(connection):
    ...
    return jsonify(
        {"nodes": all_course_names, "edges": undirected_graph, "subcategories": subcats}
    )
